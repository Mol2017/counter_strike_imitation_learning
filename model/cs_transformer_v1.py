...
# CSTransformerV1: TransformerEncoder + MLP
# Input: {'image': torch.tensor([B, T, 3, 224, 224])}
# Output: {'action': torch.tensor([B, T', 12])}
#
# - Encode each image into a 512-dim token using ResNet-18.
# - Apply temporal positional embedding to the tokens.
# - Feed the tokens into a transformer encoder.
# - Use an MLP to predict the marginal distribution of the next two actions.
class CSTransformerV1(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, inputs: dict[str, torch.tensor]) -> dict[str, torch.tensor]:
        ...
        actions = self.action_head(hidden_states[:, -1, :]) # [B, 2*12]
        actions = actions.view(n_batch, self.n_output_action, self.action_dim) # [B, 2, 12]
...
if __name__ == '__main__':
    inputs = {'image': torch.randn(1, 4, 3, 224, 224)}
